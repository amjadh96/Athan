...
// Built-in athan sources
const BUILTIN_ATHAN = {
    makkah: { name: 'الحرم المكي', src: 'audio/athan-makkah.mp3' },
    madinah: { name: 'الحرم المدني', src: 'audio/athan-madinah.mp3' },
    mishary: { name: 'مشاري العفاسي', src: 'audio/athan-mishary.mp3' },
    'abdul-basit': { name: 'عبد الباسط', src: 'audio/athan-abdul-basit.mp3' }
};
...
function updatePrayerTimesDisplay(now, times, tomorrowTimes) {
    ...
    prayers.forEach((prayer, index) => {
        ...
        if (isPassed && settings.showTomorrowTimes) {
            timeElement.textContent = tomorrowTimes[index];
...
function updateRamadanDisplay(now, times) {
    ...
    if (settings.ramadanMode) {
        ...
        document.getElementById('fasting-day').textContent = `اليوم ${getRamadanDay(now)} من 30`;
        ...
        if (settings.showImsak) {
            ...
            if (imsakMins < 0) { imsakMins += 60; imsakHours--; }
...
async function initSettings() {
    ...
    document.getElementById('auto-theme').addEventListener('change', (e) => { settings.autoTheme = e.target.checked; saveSettings(); applyTheme(); });
