...
function setupBackButton() {
    ...
    window.addEventListener('popstate', (e) => {
        if (currentScreen !== 'main-screen') {
            ...
            // Push state again to keep back button working
            history.pushState({ screen: 'main-screen' }, '', '');
        }
    });
}
...
function showScreen(screenId) {
    ...
    // Push state for back button (except for main)
...
function updateAthanIndicators() {
    ...
    prayers.forEach(prayer => {
        ...
        if (indicator) {
            indicator.style.display = settings.prayers[prayer].athan ? 'block' : 'none';
        }
    });
}
...
function updatePrayerCardStates(now, times) {
    ...
    // Update countdown label (e.g., "الظهر بعد")
...
function initSettings() {
    ...
    document.getElementById('show-tomorrow-times').addEventListener('change', (e) => {
        settings.showTomorrowTimes = e.target.checked;
        saveSettings();
        updateDisplay();
    });
