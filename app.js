...
// Athan audio URLs (users need to download these)
const ATHAN_SOURCES = {
...
function initNavigation() {
    ...
    document.querySelectorAll('.nav-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            showScreen(view === 'main' ? 'main-screen' : `${view}-screen`);
...
function updateRamadanDisplay(now, times) {
    ...
    if (settings.ramadanMode) {
        ...
        if (settings.showImsak) {
            ...
            document.getElementById('imsak-time').textContent = 
                `${String(imsakHours).padStart(2, '0')}:${String(imsakMins).padStart(2, '0')}`;
...
function checkAthanTime() {
    ...
    prayers.forEach((prayer, index) => {
        ...
        // Check exact athan time
        if (prayerSettings.athan && prayerTime === currentTime && lastAthanTime !== `${prayer}-${currentTime}`) {
            playAthan(prayer);
            lastAthanTime = `${prayer}-${currentTime}`;
        }
        
        // Check pre-reminder
        if (prayerSettings.preEnabled && prayerSettings.preBefore > 0) {
            const preTime = subtractMinutes(prayerTime, prayerSettings.preBefore);
            const reminderKey = `pre-${prayer}-${currentTime}`;
            if (preTime === currentTime && !lastReminderTime[reminderKey]) {
                playReminder();
                lastReminderTime[reminderKey] = true;
            }
        }
        
        // Check post-reminder
        if (prayerSettings.postEnabled && prayerSettings.postAfter > 0) {
            const postTime = addMinutes(prayerTime, prayerSettings.postAfter);
            const reminderKey = `post-${prayer}-${currentTime}`;
            if (postTime === currentTime && !lastReminderTime[reminderKey]) {
                playReminder();
                lastReminderTime[reminderKey] = true;
            }
        }
    ...
    // Check Suhoor reminder (30 min before Fajr)
    if (settings.ramadanMode && settings.suhoorReminder) {
        const suhoorTime = subtractMinutes(times[0], 30);
        const reminderKey = `suhoor-${currentTime}`;
        if (suhoorTime === currentTime && !lastReminderTime[reminderKey]) {
            playReminder();
            lastReminderTime[reminderKey] = true;
        }
    }
...
function initQiblaCompass() {
    ...
    // Try to use device compass
    if (window.DeviceOrientationEvent) {
        window.addEventListener('deviceorientation', (event) => {
            if (event.alpha !== null) {
                const heading = event.alpha;
                const needle = document.getElementById('qibla-needle');
                const compassAngle = qiblaDirection - heading;
                needle.style.transform = `translate(-50%, -50%) rotate(${compassAngle}deg)`;
            }
        });
    }
}
