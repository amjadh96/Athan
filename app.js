...
async function checkAthanTime() {
    ...
    for (const [index, prayer] of prayers.entries()) {
        ...
        if (currentTime === prayerTime && lastAthanTime !== currentTime) {
            console.log(`Athan time match: ${prayer} at ${prayerTime}, athan enabled: ${prayerSettings.athan}`);
            if (prayerSettings.athan && prayer !== 'sunrise') {
                console.log(`Playing athan for ${prayer}`);
...
async function playAthan(prayer) {
    ...
    console.log(`playAthan called for ${prayer}, soundId: ${soundId}`);
    ...
    if (soundId.startsWith('custom-')) {
        try {
            ...
            if (customAthan && customAthan.audio) {
                ...
                console.log('Using custom athan');
            }
        } catch (e) {
            console.log('Custom athan error, falling back:', e);
            audio.src = BUILTIN_ATHAN.makkah?.src || 'audio/makkah.mp3';
        }
    } else {
        const src = BUILTIN_ATHAN[soundId]?.src || BUILTIN_ATHAN.makkah?.src || 'audio/makkah.mp3';
        audio.src = src;
        console.log('Using builtin athan:', src);
    }
